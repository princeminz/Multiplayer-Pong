...
try:
  ...
  def time_step(game_state, dt):
    ...
    if game_state.game_on == True and game_time - start_time > 3:
      #print(game_state.ball_position,dt)
      if game_time-last_time_update>2*60:
        last_time_update = game_time
        ball_velocity_magnitude += ball_velocity_magnitude/5
        print("speed changed")
      ...
      if num_players == 2:
        ...
        y = game_state.ball_position['y'] + game_state.ball_velocity['y'] * dt*ball_velocity_magnitude
      return {"ball_position": {'x': x, 'y': y}, "ball_velocity": game_state.ball_velocity, "paddle_position": game_state.paddle_position,"player_lives": game_state.player_lives, "player_match_status": game_state.player_match_status}
  ...
  def register_player(client_address, game_state, **kwargs):
    ...
    return {"player_match_status": player_match_status, "paddle_position": paddle_position}
  ...
  def on_player_ready(connection_num,game_state,**kwargs):
    player_match_status = game_state.player_match_status
    if list(player_match_status.values()).count(2) <= 4:
  ...
  def on_game_start(game_state,**kwargs):
    ...
    for c_num, status in player_match_status.items():
      ...
      elif backend_connections[i].status != 3: 
    ...
    return {"game_on": game_on, "ball_velocity":ball_vel, "ball_position":ball_pos, "player_match_status":player_match_status, "player_lives": player_lives}
    ...
    if collided_with=="line":
      ...
      if player_lives[connection_num]==0:     
        ...
        for i in range(len(player_match_status)):
          if player_match_status[i] in [3,4]:   
            backend.server.dispatch_event("reinitGameloop", target_client=ips[i])
        if num_players==1:
          game_on = False
          player_match_status = [int(bool(player_match_status[i])) for i in range(len(player_match_status))]
